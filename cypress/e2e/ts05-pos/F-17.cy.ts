...
const realizarFluxoAteFinalizacao = (dados: F17Fixture) => {
...
describe("TS-05 — F17: Pós-submissão — Bloqueio de Edição", () => {
  let dados: F17Fixture;
  let conta: { email: string; senha: string };

  before(() => {
    cy.fixture<F17Fixture>("ts05-pos/perfil-proposta").then((f) => {
      dados = f;
    });
  ...
  context("CT-SIG-BLOC-001 — Bloqueio de edição após submissão da proposta", () => {
    it("deve exibir tela de visualização (somente leitura) ao acessar proposta submetida", () => {
      ...
      cy.get('[data-cy-index="propostas-0"]', { timeout: 15000 }).click();
